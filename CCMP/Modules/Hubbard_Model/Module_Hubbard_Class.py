...
class Hubbard:
	...
	def Plot_Eigvals_Hu(self, **kwargs):
		...
		**Kwargs : Widgets
		...
		if self.u1_checkbox.value == True:
			uniq = np.unique(np.diag(u[-1] * self.Op_nn + self.u1.value * self.Op_nn_nearest_neighbour).astype(int), return_counts=True)
	...
	def Plot_Eigvals_Ht(self, **kwargs):
		...
		**Kwargs : Widgets
		...
		if self.u1_checkbox.value == True:
			uniq = np.unique(np.diag(10 * self.Op_nn + self.u1.value * self.Op_nn_nearest_neighbour).astype(int), return_counts=True)
		elif self.u1_checkbox.value == False:
			uniq = np.unique(np.diag(self.Op_nn).astype(int), return_counts=True)
	...
	def Plot_Eigvals_H(self, **kwargs):
		...
		**Kwargs : Widgets
		...
		if self.u1_checkbox.value == True:
			uniq = np.unique(np.diag(u_max * self.Op_nn + self.u1.value * self.Op_nn_nearest_neighbour).astype(float), return_counts=True)
		elif self.u1_checkbox.value == False:
			uniq = np.unique(np.diag(self.Op_nn).astype(float), return_counts=True)
		color = mpl.cm.tab10(np.repeat(np.arange(uniq[0].size), uniq[1]) % 10)
		mpl.pyplot.rcParams["axes.prop_cycle"] = cycler("color", color)
